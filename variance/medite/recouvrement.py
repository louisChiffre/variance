    ...
    def resoudre_recouvrement(self, I):
        ...
        if tailleChAnt < tailleChPost:
            ...
            if (I[0] == 0 or I[0] == self.lg_texte1 or self.texte[I[0]-1] in sep):
                res = I[0]
                match = True
            elif (I[1] == (self.lg_texte1 - 1) or I[1] == (self.lg_texte1 + self.lg_texte2 - 1) or
                  self.texte[I[1]] in sep):
                res = I[1]
                match = True
        else:  # sinon dans l'autre
            if (I[1] == (self.lg_texte1 - 1) or I[1] == (self.lg_texte1 + self.lg_texte2 - 1) or
                    self.texte[I[1]] in sep):
                res = I[1]
                match = True
            elif (I[0] == 0 or I[0] == self.lg_texte1 or
                  self.texte[I[0]-1] in sep):
                res = I[0]
                match = True
    ...
    def addOccSeq(self, lNonOverlap):
        ...
        for longueur, debut, fin, decalageG, decalageD in lNonOverlap:  # parcours des occurrences du bloc
            ...
            if debut < fin_prec:  # debut_prec < fin_prec and
                ...
                for debut_occ_prec in locc_prec:  # pour chaque occ du bloc pr�c�dent, on va le c�surer
                ...
                try:
                    self.dicoOccLiee[hash(
                        self.texte[locc_prec[0]:locc_prec[0]+pos_cesure]), pos_cesure].extend(locc_prec)
                except KeyError:
                    self.dicoOccLiee[hash(
            ...
            if debut_suiv < fin:
                ...
                pos_cesure = fin - debut_suiv  # position de la c�sure dans le bloc suivant
                ...
                nouv_locc_suiv = []  # nouvelle liste des occurrences du bloc suivant apr�s c�sure
                ...
                try:
                    self.dicoOccLiee[hash(self.texte[nouv_locc_suiv[0]:nouv_locc_suiv[0] +
                                                     nouv_longueur_suiv]), nouv_longueur_suiv].extend(nouv_locc_suiv)
                except KeyError:
                    self.dicoOccLiee[hash(self.texte[nouv_locc_suiv[0]:nouv_locc_suiv[0] +
        ...
        for occ in lOcc:
            ...
            if d1 <= debut < fin <= f1:
                ...
                if (not overlapG and not overlapD) or (decalageG == decalageD == 0):
                    ...
                    bisect.insort_right(
                        lOverlap, (cesureD-cesureG, cesureG, cesureD, decalageG, decalageD))
