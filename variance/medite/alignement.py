...
class AlignAstarRecur(Align):
        ...
        for (cle, longueur), locc in list(dicDep.items()):
            ...
            if self.repetition(locc, self.l_texte1):
                assert texte1[locc[0]:locc[0]+longueur] == texte2[locc[-1]-self.l_texte1:locc[-1]-self.l_texte1 +
                                                                  longueur], texte1[locc[0]:locc[0]+longueur] + texte2[locc[-1]-self.l_texte1:locc[-1]-self.l_texte1+longueur]
                for occ in locc:
    ...
    def compute_alignement(self, t1, t2):
        ...
        if aligneSMEMS:
            ...
            s1, s2 = self._texteToSeqHomoNGrammes(
                t1, t2, 1, self.long_min_pivots)
    ...
    def deplacements_pond2(self, t1, t2, niveau=0):
        ...
        while (i < min(len(LResT1), len(LResT2))):
            ...
            if self.addSubDep:
                ...
                NewLResDep1 = [[x[0]+debutT1, x[1]+debutT1]
                               for x in NewLResDep1]
            if self.addSubDep:
                # self.addNumLInter(NewLResDep2,debutT2-len(nt1))
                NewLResDep2 = [
                    [x[0]+debutT2-len(nt1), x[1]+debutT2-len(nt1)] for x in NewLResDep2]
            ...
            NewLResBC2 = [
                [x[0]+debutT2-len(nt1), x[1]+debutT2-len(nt1)] for x in NewLResBC2]
            if self.addSubDep:
