...
class BiBlocList(object):
    ...
    def __init__(self, resultat, parameters, depOrdonnes=True):
        ...
        while (len_lBCT1 > 0 or len_lBCT2 > 0 or len_lRempT1 > 0 or
               len_lRempT2 > 0 or len_lSup > 0 or len_lIns > 0 or
               len_lDepT1 > 0 or len_lDepT2 > 0):
            ...
            if (len_lSup > 0 and
                ((len_lBCT1 == 0 and len_lDepT1 == 0) or
                 (len_lBCT1 == 0 and lSup[0][0] <= lDepT1[0][0]) or
                 (len_lDepT1 == 0 and lSup[0][0] < lBCT1[0][0]) or
                 (len_lBCT1 > 0 and len_lDepT1 > 0 and lSup[0][0] < lBCT1[0][0] and lSup[0][0] <= lDepT1[0][0]))):  # ajout sup
                ...
                len_lDepT1 -= len(depInBloc1)
                  ...
                  ((len_lBCT2 == 0 and len_lDepT2 == 0) or
                   (len_lBCT2 == 0 and lIns[0][0] <= lDepT2[0][0]) or
                   (len_lDepT2 == 0 and lIns[0][0] < lBCT2[0][0]) or
                   (len_lBCT2 > 0 and len_lDepT2 > 0 and lIns[0][0] < lBCT2[0][0] and lIns[0][0] <= lDepT2[0][0]))):  # ajout ins
                # décoration avec les déplacements
            ...
            # si depT1 < rempT1 et BCT1, ajout dep comme une sup
                  ...
                  ((len_lBCT1 == 0 and len_lRempT1 == 0) or
                   (len_lBCT1 == 0 and lDepT1[0][0] < lRempT1[0][0]) or
                   (len_lRempT1 == 0 and lDepT1[0][0] < lBCT1[0][0]) or
                   (len_lBCT1 > 0 and len_lRempT1 > 0 and lDepT1[0][0] < lBCT1[0][0] and lDepT1[0][0] < lRempT1[0][0]))):
                liste.append((('S', lDepT1[0][0], lDepT1[0][1], [
                             [lDepT1[0][0], lDepT1[0][1]]]), None))  # ajout du bibloc
                lDepT1.pop(0)
                len_lDepT1 -= 1
            elif (len_lDepT2 > 0 and
                  ((len_lBCT2 == 0 and len_lRempT2 == 0) or
                   (len_lBCT2 == 0 and lDepT2[0][0] < lRempT2[0][0]) or
                   (len_lRempT2 == 0 and lDepT2[0][0] < lBCT2[0][0]) or
                   (len_lBCT2 > 0 and len_lRempT2 > 0 and lDepT2[0][0] < lBCT2[0][0] and lDepT2[0][0] < lRempT2[0][0]))):  # ajout dep comme une ins
                liste.append((None, ('I', lDepT2[0][0], lDepT2[0][1], [
                             [lDepT2[0][0], lDepT2[0][1]]])))  # ajout du bibloc
                lDepT2.pop(0)
                len_lDepT2 -= 1
            elif (len_lRempT1 > 0 and len_lRempT2 > 0 and
                  ((len_lBCT1 == 0 and len_lBCT2 == 0) or
                   (lRempT1[0][0] < lBCT1[0][0] and lRempT2[0][0] < lBCT2[0][0]))):
                # décoration avec les déplacements
                depInBloc1 = self.__decoreDep(lRempT1[0], lDepT1)
                # décoration avec les déplacements
                depInBloc2 = self.__decoreDep(lRempT2[0], lDepT2)
                liste.append((('R', lRempT1[0][0], lRempT1[0][1], depInBloc1),
                              ('R', lRempT2[0][0], lRempT2[0][1], depInBloc2)))  # ajout du bibloc
                ...
                liste.append((('BC', lBCT1[0][0], lBCT1[0][1], [
                ]), ('BC', lBCT2[0][0], lBCT2[0][1], [])))  # ajout du bibloc
                lBCT1.pop(0)
                len_lBCT1 -= 1
                lBCT2.pop(0)
                len_lBCT2 -= 1
    ...
    def __decoreDep(self, intervalle, lDep):
        ...
        while len(lDep) > 0 and (lDep[0][0] >= intervalle[0]) and (lDep[0][1] <= intervalle[1]):
            i += 1
    ...
    def extractRemplacements(self):
        ...
        while i >= 0:
            ...
            if (biBloc[0] is not None and biBloc[0][0] == 'S' and
                biBlocSuiv[1] is not None and biBlocSuiv[1][0] == 'I' and
                ut.adequation_remplacement(self.texte[biBloc[0][1]:biBloc[0][2]],
                                           self.texte[biBlocSuiv[1][1]:biBlocSuiv[1][2]], ratio_min_remplacement)):
                self.liste[i:i+2] = [(('R', biBloc[0][1], biBloc[0][2], biBloc[0][3]),
    ...
    def toResultat(self):
        ...
        BCT1.extend(BCT2)
        del BCT2
        return ut.Resultat(ins, supp, depT1, rempT1,
                           self.lgSource, self.texte, BCT1, [])
    ...
    def evaluation(self, c1=0.5, c2=0.35, c3=0.15):
        ...
        x = (1.0+((bc1 + bc2 - sup - ins - remp1 -
                   remp2 - dep1 - dep2) / (seq1 + seq2)))/2.0
        pri1 = (bc1 + bc2 + dep1 + dep2) / (bc1 + bc2 +
                                            dep1 + dep2 + sup + ins + remp1 + remp2)
        ...
        return x, y, z, sim, dicoSommes, [y1, y2, y3, y4, y5], [z0, z1, z2], (pri1, pri2, pri3), sep
    ...
    def __listeToHtmlTable(self):
        ...
        for (B1, B2) in self.liste:
            ...
            if B1 is None:
                ...
                B1_type = B1[0]
                if B1_type == 'S':
                    res.extend(['<td style="background-color: #FF0000">',
                                self.__souligneTexte(B1), '</td>'])  # supp
                elif B1_type == 'R':
                    res.extend(['<td style="background-color: #0000FF">    ',
                                self.__souligneTexte(B1), '</td>'])  # remp
                elif B1_type == 'D':
                    res.extend(
                        ['<td>', self.__souligneTexte(B1), '</td>'])  # dep
                else:
                    res.extend(['<td>', self.__keepMEP(
                        self.texte[B1[1]:B1[2]]), '</td>'])  # BC
            # colonne droite
            if B2 is None:
                ...
                B2_type = B2[0]
                if B2_type == 'I':
                    res.extend(['<td style="background-color: #00FF00">',
                                self.__souligneTexte(B2), '</td>'])  # ins
                elif B2_type == 'R':
                    res.extend(['<td style="background-color: #0000FF">',
                                self.__souligneTexte(B2), '</td>'])  # remp
                elif B2_type == 'D':
                    res.extend(
                        ['<td>', self.__souligneTexte(B2), '</td>'])  # dep
                else:
                    res.extend(['<td>', self.__keepMEP(
                        self.texte[B2[1]:B2[2]]), '</td>'])  # BC
            res.append('</tr>\n')  # fin de ligne
    ...
    def __souligneTexte(self, bloc):
        ...
        if bloc[0] == 'D':
            return ('<span style="text-decoration: underline; font-weight: bold">' +
                    self.__keepMEP(self.texte[bloc[1]:bloc[2]])+'</span>')
        res = ""
        deb = i = bloc[1]
        fin = bloc[2]
        ...
        while i < fin:
            ...
            if len(lDep) > 0 and lDep[0][0] <= i:
                res += '<span style="text-decoration: underline; font-weight: bold">' + \
                    self.__keepMEP(self.texte[i:lDep[0][1]])+'</span>'
                i = lDep[0][1]
                lDep.pop(0)
        ...
        while i >= 0:
            ...
            if (B1 is not None and B1[0] == 'S'):  # bloc S
                ...
                supMoinsDep = ut.soustr_l_intervalles(
                    [[B1[1], B1[2]]], B1[3])  # bloc S moins les dep
                ratio_lissage = float(ut.longueur(
                    supMoinsDep))/(B1[2]-B1[1])  # ratio du bloc
                if ratio_lissage <= ratio_seuil_lissage:
                    self.liste[i:i +
                               1] = self.__extractDepInsSup(supMoinsDep, B1[3], 'S')
                ...
                if ratio_lissage <= ratio_seuil_lissage:
                    self.liste[i:i +
                               1] = self.__extractDepInsSup(insMoinsDep, B2[3], 'I')
            i -= 1
