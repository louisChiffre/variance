...
class BiBlocList(object):
    ...
    Un Bibloc est un tuplet de 2 Bloc align�s.
    Un Bloc est un objet (type,d�but,fin,listeDep) ou None si il est vide.
    ...
    def __init__(self, resultat, parameters, depOrdonnes=True):
        ...
        Si on utilise l'ancien algo d'identification des remplacements et d�placements,
        on peut avoir des blocs pr�sents uniquement dans resultat.getListeDeplacements().
        Ceux-ci sont alors ajout�s comme des S ou des I.

        dd: si depOrdonnes=False, pas d'assertion d'ordre sur les d�placement-> recherche d'un dep en O(n) dans __decoreDep1()
            sinon si depOrdonnes=True, assertion d'ordre respect�e -> recherche en O(log n) dans __decoreDep2()
        ...
        self.parameters = parameters  # sert seulement � l'affichage du rapport
        ...
        while (
            ...
            # if i%1000==0: logging.debug('it�ration %d',i)
            i += 1
            ...
            if len_lSup > 0 and (
                ...
                # d�coration avec les d�placements
    ...
    def __decoreDep(self, intervalle, lDep):
        """Extrait de la liste g�n�rale des d�placement lDep les d�placements se situant � l'int�rieur du bloc

        Attention !! modifie lDep et retourne res2
        Ici assertion d'ordre sur les dep, recherhe d'un dep en temps lin�aire
        comme lDep[0] est toujours >= intervalle (parce que l'ordre est maintenu par la
        fonction appelante), on commence la recherche � partir de l�
        """
        if len(lDep) == 0:
            return []
        res2 = []
    ...
    def extractRemplacements(self):
        """Recherche les S et I correspondant au crit�re de transformation en R et les convertit.

        Convertit chaque paire de bibloc (S,None) et (None,I) se suivant en un bibloc (R,R)
        Modifie directement self.liste plut�t que de recr�er une nouvelle liste
    ...
    def toResultat(self):
        ...
        Attention !! la liste des PAIRES de blocs d�plac�s n'est pas remplie"""
        supp = []
        ins = []
        rempT1 = []
        rempT2 = []
        ...
        # on extend pour former les listes des 2 fichiers pour �viter d'utliser
        # la concat�nation de liste qui cr�e une nouvelle liste
        depT1.extend(depT2)
        del depT2
    ...
    def __listeToHtmlTable(self):
        ...
        i = 0
        for B1, B2 in self.liste:
            res.append("<tr>")  # += "<tr>" # d�but de ligne
            # colonne gauche
            if B1 is None:
    ...
    def __souligneTexte(self, bloc):
        """Renvoie une chaine html avec le texte soulign� aux caract�res d�plac�s"""
        if bloc[0] == "D":
            return (
        ...
        # on parcours tout le bloc pour chercher les d�placements � l'int�rieur de celui-ci
        while i < fin:
            # si le caract�re courant est le d�but d'un d�placement
            # <= (au lieu de ==) � cause des chevauchements de d�placements du genre ('I', 5221, 5239, [(5224, 5230), (5226, 5231), (5236, 5239)])
            ...
            # si on est sur du texte normal
                ...
                i = lDep[0][0]
            else:  # si pas ou plus de d�placement
                assert len(lDep) == 0
    ...
    def __keepMEP(self, texte):
        """Fonction charg�e de conserver la mise en page du texte original

        Remplace les retours � la ligne par des <br> et les espaces par des &nbsp;

        Le remplacement des espaces par des nbsp est tr�s utile pour visulaiser des
        alignements de code source mais plus discutable pour de la langue nat.
        De m�me mais de fa�on moins importante pour les br."""
        return texte


class BiBlocListWD(BiBlocList):
    """BiblocList avec type d�placement (D) autoris�"""
    ...
    def extractDeplacements(self):
        """Extraction des d�placements

        Teste les blocs ins�r�s et supprim�s.
        Si le rapport des d�placements � l'int�rieur d'un bloc est sup�rieur au seuil
        Alors ce bloc est scind� en une liste de blocs (I ou S) et D
    ...
    def __extractDepInsSup(self, lSupOrIns, listeDep, SorI):
        ...
        while len(lSupOrIns) > 0 or len(listeDep) > 0:
            # on a ajout� tous les D, on peut ajouter le reste des (I ou S)
            if len(listeDep) == 0:
                ...
                lSupOrIns = []
            # on a ajout� tous les (I ou S), on peut ajouter les reste des D
            ...
            # si bloc courant (I ou S) <= bloc courant D alors on l'ajoute
                ...
                listeDep.pop(0)
                nbDep += 1
        # les 2 listes ont du etre trait�es enti�rement
