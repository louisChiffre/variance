...
def process(source_filepath:pathlib.Path, target_filepath:pathlib.Path, parameters:md.Parameters, output_filepath:pathlib.Path):
    ...
    root.append(
        ET.fromstring(str(z1.soup.find('teiHeader')))
    )

    medite_data = ET.SubElement(root, 'mediteData')

    # Add informations element
    ET.SubElement(medite_data, 'informations', {
    ...
    for z in res.deltas:
        if isinstance(z, BC):
            ...
            tag = z1.soup.new_tag('anchor', **{'xml:id': id_v1, 'corresp': id_v2, "function":"bc"})
            ...
            if txt.strip()=='':
                ...
                add_list(txt=remove_medit_annotations(txt), attributes=dict(corresp=target_id), name='deletion')
            ...
            add_list(txt=remove_medit_annotations(z2.txt[z.start:z.end]), attributes=dict(corresp=target_id), name='addition')
            ...
            tag = z1.soup.new_tag('metamark',function='trans', target=id_v1, corresp=id_v2)
            append_text(tag=tag, start=z.start, end=z.end)
            add_list(txt=key, attributes=dict(target=id_v1, corresp=id_v2), name='transpose')
        elif isinstance(z, DB):
            ...
            id_v1 = f'v1_{z.a_start}_{z.a_end}'
            id_v2 = f'v2_{z.b_start}_{z.b_end}'
            tag = z1.soup.new_tag('metamark',function='rempl', target=id_v1, corresp=id_v2)
            append_text(tag=tag, start=z.a_start, end=z.a_end)
            add_list(txt=remove_medit_annotations(z2.txt[z.b_start:z.b_end]), attributes=dict(target=id_v1, corresp=id_v2), name='substitution')
        else:
            breakpoint()
